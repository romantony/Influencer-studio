...
export default function SignInPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      push({ title: 'Sign in failed', description: err?.message ?? 'Please try again.', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function onGoogle() {
    try {
      ...
      push({ title: 'Google sign in failed', description: err?.message ?? 'Please try again.', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="relative hidden bg-muted lg:block">
        ...
        <div className="flex h-full items-center justify-center p-8">
          <div className="max-w-md text-center">
            <h2 className="text-3xl font-semibold">InfluencerStudio</h2>
      ...
      <div className="flex items-center justify-center p-6">
        <Card className="w-full max-w-sm">
          <CardHeader className="space-y-2 text-center">
            ...
            <CardDescription>Sign in to InfluencerStudio</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <p className="text-center text-xs text-muted-foreground">
              ...
              <Link href="/signup" className="text-primary-600">Create one</Link>
            </p>
          </CardContent>
        </Card>
      </div>
